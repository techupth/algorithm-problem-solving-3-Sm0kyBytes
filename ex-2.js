function sortStudentsByScore(students) {
  // เริ่มเขียนโค้ดตรงนี้จ้า Bubble Sort Algorithm (object in array)

  for (let i = 0; i < students.length - 1; i++) {
    for (let j = 0; j < students.length - i - 1; j++) {
      if (students[j].score < students[j + 1].score) {
        let lessValue = students[j];
        students[j] = students[j + 1];
        students[j + 1] = lessValue;
      }
    }
  }
  return students;
...
console.log(sortStudentsByScore(mockupStudents));
